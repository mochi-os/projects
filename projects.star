...
def action_data_export(a):
	...
	for row in rows:
		...
		for c in mochi.db.rows("select id, parent, author, name, content, created, edited from comments where object=? order by created, id", row["id"]) or []:
			...
			attachments = export_attachments(c["id"], project_id, a.user.identity.id, entries)
			if attachments:
				comment["attachments"] = attachments
			comments.append(comment)
		if comments:
			object["comments"] = comments
		attachments = export_attachments(row["id"], project_id, a.user.identity.id, entries)
		if attachments:
...
def delete_object_comments(object_id, project_id, preserve=False):
	comments = mochi.db.rows("select id from comments where object=?", object_id) or []
	for c in comments:
		prune_attachments(c["id"], project_id, preserve)
...
def action_comment_asset(a):
	...
	# resolve a comment author in a project the URL doesn't name.
	row = mochi.db.row("select c.author from comments c join objects o on c.object=o.id where c.id=? and o.project=?", a.input("comment"), project_id)
...
def action_activity_asset(a):
	...
	row = mochi.db.row("select a2.user from activity a2 join objects o on a2.object=o.id where a2.id=? and o.project=?", a.input("activity"), project_id)
...
def action_comment_create(a):
	...
	if project["owner"] != 1:
		...
		return {"data": {
			...
			"children": [], "attachments": attachment_list(comment_id, project_id) or [],
		}}
...
def action_comment_delete(a):
	...
	# Same binding as above: the object must be in the routed project.
	if not mochi.db.exists("select 1 from objects where id=? and project=?", object_id, project_id):
		a.error.label(404, "errors.object_not_found")
		return
...
def serve_attachment(a, variant):
	...
	# the bytes in so the project's canonical host serves its own file from
	# then on; subscribers keep pulling to cache.
	attachment_serve(a, attachment, project_id, variant=variant, member=in_project,
		adopt=project.get("owner") == 1)
...
# The project is the addressed entity; the requester must hold view access, and
# the attachment must bind to an object or comment in that project.
def event_attachment_fetch(e):
	...
	# not a project, so the container comes from the requested row: the
	# attachment's object, resolved to the project it belongs to here.
	id = e.content("id")
	project_id = ""
	if attachment_identifier(id):
		row = attachment_row(id)
		if row:
			obj = mochi.db.row("select project from objects where id=?", row["object"])
			if not obj:
				obj = mochi.db.row("select o.project from comments c join objects o on o.id=c.object where c.id=?", row["object"])
			if obj:
	...
	# The project's own entity may always fetch bytes bound to it - that is
	# the owner taking a subscriber's upload in (attachment_accept / adopt).
	attachment_respond(e, project_id,
		lambda sender, container: sender == container or check_project_access(sender, container, "view"),
		member=in_project)
...
def event_attachment_push(e):
	...
	mochi.log.debug("attachment/push: to=%s from=%s object=%s id=%s", project_id, e.header("from"), e.content("object"), e.content("id"))
...
def action_attachment_list(a):
	...
	attachments = attachment_list(object_id, project_id) or []
...
def action_attachment_create(a):
	...
	if project["owner"] != 1:
		...
		if not attachment_push(project_id, object_id, attachments, a.user.identity.id):
			for att in attachments:
				attachment_delete(att["id"])
			a.error.label(502, "errors.could_not_reach_project_owner")
			return
...
def insert_schema(project_id, schema):
	...
	for obj in (schema.get("objects") or []):
		...
		if obj_atts:
			attachment_store(obj_atts, project_id, obj.get("id", ""))
		...
		for c in (obj.get("comments") or []):
			...
			if c_atts:
				attachment_store(c_atts, project_id, c.get("id", ""))
	...
	for row in (mochi.db.rows("select id from objects where project=?", project_id) or []):
		if row["id"] not in object_survivors:
			delete_object_local(project_id, row["id"], True)
	for row in (mochi.db.rows("select c.id from comments c join objects o on c.object=o.id where o.project=?", project_id) or []):
		if row["id"] not in comment_survivors:
			prune_attachments(row["id"], project_id, True)
	...
	for obj in (schema.get("objects") or []):
		...
		for att in (attachment_list(identifier, project_id) or []):
...
def event_comment_submit(e):
	...
	if attachments:
		attachment_accept(attachments, sender, comment_id, project_id)
	object_set(object_id, {"updated": now})
...
def do_comment_create(project_id, project, params, user_id, user_name):
	...
	comment_attachments = attachment_list(comment_id, project_id) or []
	comment_event = {
		"project": project_id, "object": object_id, "id": comment_id,
