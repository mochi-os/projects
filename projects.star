...
def database_upgrade(version):
	...
	if version == 3 or version == 4:
		...
		# create the table and copy existing rows across the transition
		# bridge, aborting without advancing if the bridge is unavailable.
		# Both calls are idempotent, so the step is safe to run at either
		# version - 4 covers databases whose version-3 run was consumed on a
		# server predating the bridge (needs core 0.4.228).
...
# whole export.
def export_attachments(object_id, project_id, frm):
	result = []
...
def action_data_export(a):
	...
	for row in rows:
		...
		for c in mochi.db.rows("select id, parent, author, name, content, created, edited from comments where object=? order by created, id", row["id"]) or []:
			...
			attachments = export_attachments(c["id"], project_id, a.user.identity.id)
			if attachments:
				comment["attachments"] = attachments
			comments.append(comment)
		if comments:
			object["comments"] = comments
		attachments = export_attachments(row["id"], project_id, a.user.identity.id)
		if attachments:
...
def event_attachment_fetch(e):
	...
	attachment_respond(e, project_id,
		lambda sender, container: check_project_access(sender, container, "view"),
		member=in_project)
