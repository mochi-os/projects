...
import type { ProjectType, ProjectField, FieldOption, ProjectView } from '@/types'
...
export function DesignPreview({
  ...
  const cardFields = boardView.cardfields?.split(',').filter(Boolean) || ['title']
    ...
    return (
      <div className="p-3 bg-background border rounded-lg shadow-sm space-y-2">
        ...
        {cardFields.map((fieldId) => {
          ...
          return (
            <div key={fieldId}>{renderFieldValue(field, value)}</div>
          )
        })}
      </div>
    )
        ...
        {columnOptions.slice(0, 4).map((opt) => (
          ...
          >
            ...
            <div className="space-y-2">
              {SAMPLE_CARDS.filter(
                (c) => c.values[columnField as keyof typeof c.values] === opt.id
              ).map((card) => (
        ...
        <div className="flex items-center bg-muted/50 text-sm font-medium text-muted-foreground">
          ...
          {cardFields.map((fieldId) => {
            ...
            return (
              <div
                key={fieldId}
                className={cn('px-3 py-2', fieldId === 'title' ? 'flex-1' : 'w-28')}
              >
                {field?.name || fieldId}
              </div>
            )
