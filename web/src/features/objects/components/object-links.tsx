// Mochi Projects: Object link display and management
// Copyright © 2026 Mochisoft OÜ
// SPDX-License-Identifier: AGPL-3.0-only
// This file is part of Mochi, licensed under the GNU AGPL v3 with the
// Mochi Application Interface Exception - see license.txt and license-exception.md.

import { EntityObjectLinks, type EntityObjectLinksProps } from "@mochi/web";
import projectsApi from "@/api/projects";
import type { ProjectObject } from "@/types";

type ObjectLinksProps = Omit<
  EntityObjectLinksProps<ProjectObject>,
  "containerId" | "listObjects" | "createLink" | "deleteLink"
> & { projectId: string };

export function ObjectLinks({ projectId, ...props }: ObjectLinksProps) {
  return (
    <EntityObjectLinks
      {...props}
      containerId={projectId}
      listObjects={projectsApi.listObjects}
      createLink={projectsApi.createLink}
      deleteLink={projectsApi.deleteLink}
    />
  );
}
